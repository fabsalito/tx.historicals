Calendar=
			function(mondayFirst,dateStr,onSelected,onClose){
				this.activeDiv=null;
...
Calendar.cellClick=
			...
			function(_par) {
			    ...
			    div.setAttribute("style", "z-index:9999999;");
			    if (this.isPopup) {
			    ...
			    var cal = this;
			    var hh =
						function(text, cs, navtype) {
						    cell = Calendar.createElement("td", row);
						    cell.colSpan = cs; cell.className = "button";
...
Calendar.prototype.setMondayFirst=
			...
			function(str,fmt){
				...
				for(i=0;i<a.length;++i){
					if(a[i].search(/[a-zA-Z]+/)!=-1){
						...
						if(parseInt(a[i])<=12&&m==-1){
							m=a[i]-1;
						}
						else{
							if(parseInt(a[i])>31&&y==0){
								y=a[i];
							}
							else{
								if(d==0){
...
Calendar.prototype._dragStart=
			function(ev){
				...
				var posY;
				if(Calendar.is_ie){
					posY=window.event.clientY+document.body.scrollTop;posX=window.event.clientX+document.body.scrollLeft;
				}
				else{
...
Calendar._TT["PREV_YEAR"] = "Año anterior (menú desplegable)";
Calendar._TT["PREV_MONTH"] = "Mes anterior (menú desplegable)";
Calendar._TT["GO_TODAY"] = "Ir a Hoy";
Calendar._TT["NEXT_MONTH"] = "Mes próximo (menú desplegable)";
Calendar._TT["NEXT_YEAR"] = "Año próximo (menú desplegable)";
